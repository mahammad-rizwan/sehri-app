...
export default function AdminDonationHistory() {
  ...
  // Keep filtered list in sync with donations + zone filter
  useEffect(() => {
    setFilteredDonations(
      selectedZone === 'all'
        ? donations
        : donations.filter(d => d.donor_zone === selectedZone)
    );
  }, [selectedZone, donations]);

  // ── Update totals locally based on status change ──────────────
  const updateLocalTotals = (donation: Donation, newStatus: 'paid' | 'rejected', newAmount: number | null) => {
    setData((prev: any) => {
      ...
      if (newStatus === 'paid') {
        totalAmount    += Number(newAmount || 0);
        totalDonations += 1;
      }
  ...
  const executeStatusChange = async (donation: Donation, newStatus: 'paid' | 'rejected', amount?: string) => {
    const id = donation.id + newStatus;
    setActionLoading(id);
    try {
      ...
      setDonations(prev => prev.map(d =>
        d.id === donation.id
          ? { ...d, status: newStatus, amount: newAmount ?? d.amount }
          : d
      ));

      // Update totals without refetch
  ...
  const handleAccept = (donation: Donation) => {
    // Pre-fill with user-declared amount if available
  ...
  const handleReject = (donation: Donation) => {
    Alert.alert(
      '❌ Reject Donation',
      `Reject donation from ${donation.donor_name}?${donation.status === 'paid' ? '\n\nThis will deduct ₹' + Number(donation.amount || 0).toLocaleString() + ' from the total.' : ''}`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Reject',
          style: 'destructive',
          onPress: () => executeStatusChange(donation, 'rejected'),
        },
      ]
    );
  };

  // ── Confirm amount and accept ─────────────────────────────────
  const confirmAccept = () => {
          ...
          <LinearGradient colors={['rgba(201,168,76,0.15)', 'rgba(201,168,76,0.03)']} style={styles.statsCard}>
            <View style={styles.statsRow}>
              ...
              <View style={styles.stat}>
                ...
                <Text style={styles.statLabel}>Paid Donations</Text>
              </View>
            </View>
          </LinearGradient>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
            ...
            {zones.map((z) => (
              ...
              >
                ...
                <Text style={[styles.filterText, selectedZone === z && styles.filterTextActive]}>{zoneLabel(z)}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>

          {/* Donations List */}
          <Text style={styles.sectionTitle}>Recent Transactions ({filteredDonations.length})</Text>
            ...
            filteredDonations.map((d) => {
              ...
              return (
                <LinearGradient
                  ...
                  style={[styles.donationCard, d.status === 'paid' && styles.cardPaid, d.status === 'rejected' && styles.cardRejected]}
                >
                  {/* Header Row */}
                  ...
                  <View style={styles.amountRow}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.amountValue}>
                        {d.amount ? `₹${Number(d.amount).toLocaleString()}` : 'Not set'}
                      </Text>
                    </View>
                  ...
                  {/* View Proof */}
                  <TouchableOpacity style={styles.proofBtn} onPress={() => openProof(d)} activeOpacity={0.75}>
                  ...
                  {isActing ? (
                    <View style={styles.actingRow}>
                      ...
                      <Text style={styles.actingText}>Updating...</Text>
                    </View>
                  ) : (
                    <View style={styles.actionRow}>
                      {/* Accept — show for pending & rejected */}
                      {d.status !== 'paid' && (
                        <TouchableOpacity
                        ...
                        >
                          ...
                          <Text style={styles.acceptBtnText}>
                            {d.status === 'rejected' ? 'Re-accept' : 'Accept'}
                          </Text>
                        </TouchableOpacity>
                      )}

                      {/* Reject — show for pending & paid */}
                      {d.status !== 'rejected' && (
                        <TouchableOpacity
                        ...
                        >
                          ...
                          <Text style={styles.rejectBtnText}>
                            {d.status === 'paid' ? 'Undo / Reject' : 'Reject'}
                          </Text>
                        </TouchableOpacity>
                      )}
                    </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.amountModalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>✅ Confirm Amount</Text>
              <TouchableOpacity onPress={() => { setAmountModalVisible(false); setPendingAction(null); }} activeOpacity={0.7}>
                <Ionicons name="close" size={24} color={COLORS.textMuted} />
              </TouchableOpacity>
            </View>
            <Text style={styles.modalSubtitle}>
              {pendingAction?.donation.donor_name} • {zoneEmoji(pendingAction?.donation.donor_zone || '')} {zoneLabel(pendingAction?.donation.donor_zone || '')}
            </Text>
            <Text style={styles.amountInputLabel}>Enter verified amount (₹)</Text>
            <TextInput
            ...
            {pendingAction?.donation.amount ? (
              <Text style={styles.amountHint}>
                User declared: ₹{Number(pendingAction.donation.amount).toLocaleString()}
              </Text>
            ) : null}
            <View style={styles.amountModalActions}>
              <TouchableOpacity
                style={styles.cancelBtn}
                onPress={() => { setAmountModalVisible(false); setPendingAction(null); }}
                activeOpacity={0.8}
              >
                <Text style={styles.cancelBtnText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.confirmBtn} onPress={confirmAccept} activeOpacity={0.8}>
                <Ionicons name="checkmark-circle" size={18} color="#fff" />
                <Text style={styles.confirmBtnText}>Accept & Add to Total</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {selectedProof && (
              <ScrollView style={styles.proofScroll} contentContainerStyle={styles.proofScrollContent} showsVerticalScrollIndicator={false}>
                <Image source={{ uri: selectedProof.url }} style={styles.proofImage} resizeMode="contain" />
              </ScrollView>
            )}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  cardPaid:      { borderColor: 'rgba(76,175,80,0.3)' },
